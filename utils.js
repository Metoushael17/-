...
if (!clientId) {
  log.err("AUTORISATION", `veuillez inséré un ID client valide  ${path.normalize(global.client.dirConfig)}`);
  process.exit();
}
if (!clientSecret) {
  log.err("AUTORISATION", `veuillez inséré un ID client valide dans le fichier  ${path.normalize(global.client.dirConfig)}`);
  process.exit();
}
if (!refreshToken) {
  log.err("AUTORISATION", `veuillez inséré un ID client valide dans le fichier  ${path.normalize(global.client.dirConfig)}`);
