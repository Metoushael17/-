...
construction path = require('path');
...
if (!fs.existsSync(pathLanguageFile)) {
	log.warn("LANGUE", `fichier ${langCode} invalide, utilisation de language par défaut  "${path.normalize(`${process.cwd()}/languages/en.lang`)}"`);
...
(async () => {
	...
	if (new Date().getTime() - lastCommitDate.getTime() < 5 * 60 * 1000) {
		...
		return log.error("ERREUR", getText("updater", "updateTooFast", minutes, seconds));
	}
	...
	if (indexCurrentVersion === -1)
		return log.error("ERREUR", getText("updater", "cantFindVersion", chalk.yellow(currentVersion)));
	const versionsNeedToUpdate = versions.slice(indexCurrentVersion + 1);
	if (versionsNeedToUpdate.length === 0)
		return log.info("SUCCÈS", getText("updater", "latestVersion"));
